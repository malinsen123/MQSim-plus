# MQSim-plus: A Simulator for Ultra-High-IOPS SSDs

MQSim-plus is a simulator for ultra-high IOPS SSDs. It is built on top of the original MQSim simulator. The simulator is designed to help researchers and engineers evaluate the performance of SSDs under various workloads and configurations. It provides a flexible and extensible framework for modeling different aspects of SSD architecture, including host interfaces, memory types, caching mechanisms, and garbage collection policies. The original repository is at https://github.com/CMU-SAFARI/MQSim.git

## Ultra-High-IOPS SSDs
Ultra-high IOPS SSDs are a new generation of solid-state drives that are designed to deliver extremely high input/output operations per second (IOPS) performance and unleash the full potential of NAND flash memory over 512B~4KB page sizes. These SSDs are capable of handling a large number of concurrent I/O requests, making them ideal for data-intensive applications such as databases, virtualization, and cloud computing.

## MQSim-plus Features
1. Modeling of modern NAND flash memory devices: Support the new SCA(separate command address) protocal in modern NAND flash memory chips.
2. Request scheduling to better exploit NAND flash operational parallelism: Suppport the Independent multi-plane read.
3. Emulation of the effect of concatenated ECC: Support two-layer 512B/4KB concatenated ECC.
...
```
$ make
$ ./MQSim-plus -i <SSD Configuration File> -w <Workload Definition File>
```


## Original MQSim Execution Configurations 
...
## MQSim-plus enhanced Execution Configurations 

1. **SCA_Enabled:** the toggle to enable SCA protocol. Range = {true, false}.
2. **Independent_Plane_Read_Enabled:** the toggle to enable independent plane read. Range = {true, false}.
3. **Two_Layer_ECC_Enabled:** the toggle to enable two-layer ECC. Range = {true, false}.
4. **Two_Layer_ECC_512B_Failure_Rate:** the failure rate of 512B layer ECC. Range = {all positive double precision values}.


## MQSim-plus Workload Definition
MQsim-plus utilize the same workload definition file as the original MQSim simulator. The workload definition file is used to define the workloads that are executed by the simulator. You can define different sets of workloads in the workload definition file. Each set of workloads is called an *I/O scenario*. Each I/O scenario is executed separately and the results are written into a separate output file. You can define your preferred set of workloads in the XML format. If the specified workload definition file does not exist, MQSim will create a sample workload definition file in XML format for you (i.e., workload.xml). Here is the explanation of the XML attributes and tags for the workload definition file:
...
## Analyze MQSim-plus's XML Output
