	...
	namespace FlashMemory
	{
		...
		public:
			...
			void EndCMDXfer(Flash_Command* command)//End transferring write data to the Flash chip
			{
				...
				this->STAT_totalXferinTime += (Simulator->Time() - this->lastTransferStart);
				...
				this->Dies[command->Address[0].DieID]->STAT_TotalXferinTime += (Simulator->Time() - lastTransferStart);
			...
			{
				//this->STAT_totalXferTime += (Simulator->Time() - this->lastTransferStart);
				//std::cout<<"EndDataOutXfer"<<std::endl;
				//std::cout<<"The plus time is: "<<(Simulator->Time() - this->lastTransferStart)<<std::endl;

				this->STAT_totalXferoutTime += (Simulator->Time() - this->lastTransferStart);
				if (this->idleDieNo != die_no)
					STAT_totalOverlappedXferExecTime += (Simulator->Time() - lastTransferStart);
				//this->Dies[command->Address[0].DieID]->STAT_TotalXferTime += (Simulator->Time() - lastTransferStart);
				this->Dies[command->Address[0].DieID]->STAT_TotalXferoutTime += (Simulator->Time() - lastTransferStart);
				this->lastTransferStart = INVALID_TIME;
			}
		...
		private:
			...
			//LM split dataxferin and dataxferout
			//sim_time_type STAT_totalExecTime, STAT_totalXferTime, STAT_totalOverlappedXferExecTime;
			sim_time_type STAT_totalExecTime, STAT_totalXferinTime, STAT_totalXferoutTime,  STAT_totalOverlappedXferExecTime;
