...
namespace NVM
{
	namespace FlashMemory
	{
		Flash_Chip::Flash_Chip(const sim_object_id_type& id, flash_channel_ID_type channelID, flash_chip_ID_type localChipID,
			...
			STAT_totalExecTime(0), STAT_totalXferinTime(0), STAT_totalXferoutTime(0), STAT_totalOverlappedXferExecTime(0)
		{
		...
		void Flash_Chip::start_command_execution(Flash_Command* command)
		{
			...
			//std::cout<<"executing latency: "<<Get_command_execution_latency(command->CommandCode, command->Address[0].PageID)<<std::endl;
		...
		void Flash_Chip::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
		{
			...
			//attr = "Total_XFER_Time";
			//val = std::to_string(STAT_totalXferinTime + STAT_totalXferoutTime);
			//xmlwriter.Write_attribute_string_inline(attr, val);

			sim_time_type STAT_totalXferTime = STAT_totalXferinTime + STAT_totalXferoutTime;

			attr = "Fraction_of_Time_in_DataXferin";
			val = std::to_string(STAT_totalXferinTime / double(Simulator->Time()));
			xmlwriter.Write_attribute_string_inline(attr, val);

			attr = "Fraction_of_Time_in_DataXferout";
			val = std::to_string(STAT_totalXferoutTime / double(Simulator->Time()));
			xmlwriter.Write_attribute_string_inline(attr, val);
			


			attr = "Total_Overlapped_XFER_EXEC_Time";
