...
bool Device_Parameter_Set::SCA_Enabled = false;
bool Device_Parameter_Set::Independent_Plane_Read_Enabled = false;
bool Device_Parameter_Set::Two_Layer_ECC_Enabled = true;
double Device_Parameter_Set::Two_Layer_ECC_512B_Failure_Rate = 0.00001;
...
void Device_Parameter_Set::XML_serialize(Utils::XmlWriter& xmlwriter)
{
	...
	//SCA
	attr = "SCA_Enabled";
	val = (SCA_Enabled ? "true" : "false");
	xmlwriter.Write_attribute_string(attr, val);

	//LM
	//Independent_Plane_Read_Enabled
	attr = "Independent_Plane_Read_Enabled";
	val = (Independent_Plane_Read_Enabled ? "true" : "false");
	xmlwriter.Write_attribute_string(attr, val);
   
	//LM
	//Two_Layer_ECC_Enabled
	attr = "Two_Layer_ECC_Enabled";
	val = (Two_Layer_ECC_Enabled ? "true" : "false");
	xmlwriter.Write_attribute_string(attr, val);

	attr = "Two_Layer_ECC_512B_Failure_Rate";
	val = std::to_string(Two_Layer_ECC_512B_Failure_Rate);
	xmlwriter.Write_attribute_string(attr, val);
...
void Device_Parameter_Set::XML_deserialize(rapidxml::xml_node<> *node)
{
	try
	{
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			...
			} else if (strcmp(param->name(), "SCA_Enabled") == 0) {
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper);
				SCA_Enabled = (val.compare("FALSE") == 0 ? false : true);
			} else if (strcmp(param->name(), "Independent_Plane_Read_Enabled") == 0) {
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper);
				Independent_Plane_Read_Enabled = (val.compare("FALSE") == 0 ? false : true);
