...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
{
	...
	device->SCA_enabled = parameters->SCA_Enabled;
	device->Independent_plane_read_enabled = parameters->Independent_Plane_Read_Enabled;
	device->Two_layer_ECC_enabled = parameters->Two_Layer_ECC_Enabled;
	device->Two_layer_ECC_512B_failure_rate = parameters->Two_Layer_ECC_512B_Failure_Rate;

	/* LM debug
	if(SCA_enabled)
	{
		//LM: SCA is enabled, so we need to set the read command transfer time for 1 plane, 2 planes, 3 planes, and 4 planes to 60
		//LM: This is done in the constructor of ONFI_Channel_NVDDR2
	
		std::cout << "SCA is enabled" << std::endl;
	}else
	{
		std::cout << "SCA is disabled" << std::endl;
	}
	

	//LM debug independent plane read
	...
	{
		std::cout << "Independent plane read is disabled" << std::endl;
	}
	...
	case NVM::NVM_Type::FLASH:
	{
		...
		case SSD_Components::ONFI_Protocol::NVDDR2:
		{
			...
			for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++)
			{
				...
				*/
				channels[channel_cntr] = new SSD_Components::ONFI_Channel_NVDDR2(channel_cntr, parameters->Chip_No_Per_Channel,
																				 chips, parameters->Flash_Channel_Width,
																				 (sim_time_type)((double)1000000 / parameters->Channel_Transfer_Rate) * 2, (sim_time_type)((double)1000000 / parameters->Channel_Transfer_Rate) * 2, parameters->SCA_Enabled);
			...
			device->PHY = new SSD_Components::NVM_PHY_ONFI_NVDDR2(device->ID() + ".PHY", channels, parameters->Flash_Channel_Count, parameters->Chip_No_Per_Channel,
																  parameters->Flash_Parameters.Die_No_Per_Chip, parameters->Flash_Parameters.Plane_No_Per_Die, parameters->Two_Layer_ECC_Enabled, parameters->Two_Layer_ECC_512B_Failure_Rate);
		...
		case SSD_Components::Flash_Scheduling_Type::OUT_OF_ORDER:
			tsu = new SSD_Components::TSU_OutOfOrder(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2 *>(device->PHY),
													 ...
													 erase_suspension, program_suspension, parameters->Independent_Plane_Read_Enabled);
			break;
