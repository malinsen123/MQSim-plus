																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	void IO_Flow_Base::NVMe_consume_io_request(Completion_Queue_Entry* cqe)
	{
		...
		//LM modification comment this out
		nvme_queue_pair.Submission_queue_head = cqe->SQ_Head;

		//std::cout<<"inside flow base NVMe_consume_io_request"<<std::endl;
		//std::cout<<"nvme_queue_pair.Submission_queue_head is "<<nvme_queue_pair.Submission_queue_head<<std::endl;
	...
	Submission_Queue_Entry* IO_Flow_Base::NVMe_read_sqe(uint64_t address)
	{
		...
		//std::cout<<"Time is "<<Simulator->Time()<<std::endl;
		//std::cout<<"the request index is "<<(uint16_t)((address - nvme_queue_pair.Submission_queue_memory_base_address) / sizeof(Submission_Queue_Entry))<<std::endl;

		//std::cout<<"!!the submission head is "<<nvme_queue_pair.Submission_queue_head<<std::endl;
		//std::cout<<"!!the submission tail is "<<nvme_queue_pair.Submission_queue_tail<<std::endl;
		//std::cout<<"the completion head is "<<nvme_queue_pair.Completion_queue_head<<std::endl;
		//std::cout<<"the completion tail is "<<nvme_queue_pair.Completion_queue_tail<<std::endl;
		...
		if (request->Type == Host_IO_Request_Type::READ) {
			...
			//std::cout<<"the time is "<<Simulator->Time()<<std::endl;
			//std::cout<<"NVMe_read_sqe: the request type is write"<<std::endl;
		...
		//LM modification
		//nvme_queue_pair.Submission_queue_head = (nvme_queue_pair.Submission_queue_head + 1) % nvme_queue_pair.Submission_queue_size;
	...
	void IO_Flow_Base::Submit_io_request(Host_IO_Request* request)
	{
		...
		//std::cout<<"nvme_queue_pair size is "<<nvme_queue_pair.Submission_queue_size<<std::endl;
		//utilizing the head and tail pointers to calculate the real size of the queue
		uint16_t real_size = (nvme_queue_pair.Submission_queue_tail - nvme_queue_pair.Submission_queue_head + nvme_queue_pair.Submission_queue_size) % nvme_queue_pair.Submission_queue_size;
		//std::cout<<"the head is "<<nvme_queue_pair.Submission_queue_head<<std::endl;
		//std::cout<<"the tail is "<<nvme_queue_pair.Submission_queue_tail<<std::endl;
		//std::cout<<"the submission tail register address is "<<nvme_queue_pair.Submission_tail_register_address_on_device<<std::endl;
		//std::cout<<"the completion head register address is "<<nvme_queue_pair.Completion_head_register_address_on_device<<std::endl;
		//std::cout<<"real size is "<<real_size<<std::endl;
	...
	void IO_Flow_Base::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		attr = "Totol_Time";
		val = std::to_string((double)Simulator->Time() / SIM_TIME_TO_SECONDS_COEFF);
		xmlwriter.Write_attribute_string(attr, val);
