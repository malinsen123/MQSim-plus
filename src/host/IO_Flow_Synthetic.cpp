																														  ...
																														  seed(seed), generate_aligned_addresses(generate_aligned_addresses), alignment_value(alignment_value)
{
	...
	/*
	Submission_Queue_Entry* IO_Flow_Synthetic::NVMe_read_sqe(uint64_t address)
	{
		Submission_Queue_Entry* sqe = IO_Flow_Base::NVMe_read_sqe(address);

		std::cout<<"in the read_sqe function in synthetic"<<std::endl;
		std::cout<<"queue head: "<<nvme_queue_pair.Submission_queue_head<<std::endl;
		std::cout<<"queue tail: "<<nvme_queue_pair.Submission_queue_tail<<std::endl;

		// the real size of the submission queue the difference between the head and the tail
		...
		if (generator_type == Utils::Request_Generator_Type::QUEUE_DEPTH) {
			...
			//for(int i = 0; i < average_number_of_enqueued_requests - real_size; i++)			
			//{
	...
	*/
	...
	void IO_Flow_Synthetic::NVMe_consume_io_request(Completion_Queue_Entry* io_request)
	{
		...
		//std::cout<<"STAT_serviced_request_count: "<<STAT_serviced_request_count<<std::endl;
