...
int main(int argc, char* argv[])
{
	...
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {
		...
		//LM old output file name without SSD_Device_Configuration
		...
		//LM new output file name with SSD_Device_Configuration
		collect_results(ssd, host, (workload_defs_file_path.substr(0, workload_defs_file_path.find_last_of(".")) + "_ "+ ssd_config_file_path.substr(0, ssd_config_file_path.find_last_of("."))   +"_scenario_" + std::to_string(cntr) + ".xml").c_str());            
	
	}
    cout << "Simulation complete; No need to press any key to exit." << endl;
