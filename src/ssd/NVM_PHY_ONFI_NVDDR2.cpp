...
namespace SSD_Components {
	...
	NVM_PHY_ONFI_NVDDR2::NVM_PHY_ONFI_NVDDR2(const sim_object_id_type& id, ONFI_Channel_NVDDR2** channels,
		unsigned int ChannelCount, unsigned int chip_no_per_channel, unsigned int DieNoPerChip, unsigned int PlaneNoPerDie, bool Two_layer_ECC_enabled ,double Two_layer_ECC_512B_failure_rate)
		: NVM_PHY_ONFI(id, ChannelCount, chip_no_per_channel, DieNoPerChip, PlaneNoPerDie), channels(channels)
	{
		...
		for (unsigned int channelID = 0; channelID < channel_count; channelID++) {
			...
			for (unsigned int chipID = 0; chipID < chip_no_per_channel; chipID++) {
				...
				bookKeepingTable[channelID][chipID].Two_layer_ECC_enabled = Two_layer_ECC_enabled;
				bookKeepingTable[channelID][chipID].Two_layer_ECC_512B_failure_rate = Two_layer_ECC_512B_failure_rate;
		...
		switch (transaction_list.front()->Type) {
			case Transaction_Type::READ:
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
					...
					//std::cout<<"the size of the transaction list is: "<<transaction_list.size()<<std::endl;
					//std::cout<<"the read command time is: "<<target_channel->ReadCommandTime[transaction_list.size()]<<std::endl;
					...
					//LM test for SCA
					//chipBKE->Last_transfer_finish_time = Simulator->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()];
					//Simulator->Register_sim_event(Simulator->Time() + suspendTime, this,
					//	dieBKE, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED);
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					...
					//LM test for write
					//std::cout<<"the size of the transaction list is: "<<transaction_list.size()<<std::endl;
					//std::cout<<"the program command time is: "<<target_channel->ProgramCommandTime[transaction_list.size()]<<std::endl;
	...
	inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE, NVM_Transaction_Flash* tr)
	{
		...
		//Determine if two layer ecc is enabled
		if(chipBKE->Two_layer_ECC_enabled)
		{
			//Based on the two layer ecc 512B failure rate make a random decision with the possibility of Two_layer_ECC_512B_failure_rate setting the Data and metadata size to ECC_SIZE_IN_BYTE and (1-Two_layer_ECC_512B_failure_rate) 
			//ratio setting the Data and metadata size to the original size (try to utilize std standard library)
			...
			if (chance < chipBKE->Two_layer_ECC_512B_failure_rate)
			{
				// Simulate a failure — set size to ECC_SIZE_IN_BYTE
				tr->Data_and_metadata_size_in_byte = ECC_SIZE_IN_BYTE;
			}
			else
			{
				// Keep original size
				tr->Data_and_metadata_size_in_byte = tr->Data_and_metadata_size_in_byte;
			}

		}
		else
		{   if(tr->Data_and_metadata_size_in_byte<= ECC_SIZE_IN_BYTE)
			tr->Data_and_metadata_size_in_byte = ECC_SIZE_IN_BYTE;
		}
		...
		//std::cout<<"the dataout transfer time is: "<<NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, channels[tr->Address.ChannelID])<<std::endl;
		//std::cout<<"the size of the transaction is: "<<tr->Data_and_metadata_size_in_byte<<std::endl;
