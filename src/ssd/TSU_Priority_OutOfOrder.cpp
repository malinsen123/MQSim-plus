...
TSU_Priority_OutOfOrder::TSU_Priority_OutOfOrder(const sim_object_id_type &id,
                                                ...
                                                bool ProgramSuspensionEnabled, bool Independent_plane_read_enabled)
    : TSU_Base(id,
               ftl,
               NVMController,
               ...
               //ProgramSuspensionEnabled)
                ProgramSuspensionEnabled, Independent_plane_read_enabled)
{
...
void TSU_Priority_OutOfOrder::Schedule()
{
    ...
    //std::cout<<"transaction_receive_slots.size() is "<<transaction_receive_slots.size()<<std::endl;
        ...
        case Transaction_Type::READ:
            ...
            case Transaction_Source_Type::USERIO:
                if ((*it)->Priority_class != IO_Flow_Priority_Class::UNDEFINED)
                {
                    ...
                    //std::cout<<"the size of UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][static_cast<int>((*it)->Priority_class)] is "<<UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][static_cast<int>((*it)->Priority_class)].size()<<std::endl;
                
                    //only print when channelID = 0 and chipID = 0
                    /*
                    if((*it)->Address.ChannelID == 0 && (*it)->Address.ChipID == 0)
                    {
                        std::cout<<"the size of UserReadTRQueue[0][0][HIGH] is "<<UserReadTRQueue[(*it)->Address.ChannelID][(*it)->Address.ChipID][static_cast<int>((*it)->Priority_class)].size()<<std::endl;
                    }
                    */
...
bool TSU_Priority_OutOfOrder::service_read_transaction(NVM::FlashMemory::Flash_Chip *chip)
{
    ...
    //std::cout<<"TSU_Priority_OutOfOrder::service_read_transaction: sourceQueue1->size() = "<<sourceQueue1->size()<<std::endl;
    //std::cout<<"chip->ChannelID = "<<chip->ChannelID<<", chip->ChipID = "<<chip->ChipID<<std::endl;
    ...
    issue_command_to_chip(sourceQueue1, sourceQueue2, Transaction_Type::READ, suspensionRequired, independent_plane_read_enabled);
...
bool TSU_Priority_OutOfOrder::service_write_transaction(NVM::FlashMemory::Flash_Chip *chip)
{
    ...
    issue_command_to_chip(sourceQueue1, sourceQueue2, Transaction_Type::WRITE, suspensionRequired, independent_plane_read_enabled);
...
bool TSU_Priority_OutOfOrder::service_erase_transaction(NVM::FlashMemory::Flash_Chip *chip)
{
    ...
    issue_command_to_chip(source_queue, NULL, Transaction_Type::ERASE, false, independent_plane_read_enabled);
