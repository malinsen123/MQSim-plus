...
namespace SSD_Components
{
	...
	TSU_Base::TSU_Base(const sim_object_id_type& id, FTL* ftl, NVM_PHY_ONFI_NVDDR2* NVMController, Flash_Scheduling_Type Type,
		...
		//sim_time_type EraseReasonableSuspensionTimeForWrite) OLd
		sim_time_type EraseReasonableSuspensionTimeForWrite, bool Independent_plane_read_enabled)
		...
		eraseReasonableSuspensionTimeForWrite(EraseReasonableSuspensionTimeForWrite), opened_scheduling_reqs(0), independent_plane_read_enabled(Independent_plane_read_enabled)
	{
		_my_instance = this;
	...
	//LM should modify for independent plane read
	...
	bool TSU_Base::issue_command_to_chip(Flash_Transaction_Queue *sourceQueue1, Flash_Transaction_Queue *sourceQueue2, Transaction_Type transactionType, bool suspensionRequired, bool independent_plane_read_enabled)
	{
		...
		//std::cout<<"begin issue_command_to_chip"<<std::endl;
		//std::cout<<"the size of sourceQueue1 is "<<sourceQueue1->size()<<std::endl;
		...
		for (unsigned int i = 0; i < die_no_per_chip; i++)
		{
			...
			for (Flash_Transaction_Queue::iterator it = sourceQueue1->begin(); it != sourceQueue1->end();)
			{
				if (transaction_is_ready(*it) && (*it)->Address.DieID == dieID && !(planeVector & 1 << (*it)->Address.PlaneID))
				{
					...
					//std::cout<<"the planevector is "<<planeVector<<std::endl;
					//std::cout<<"the address planeid is "<<(*it)->Address.PlaneID<<std::endl;
					//if (planeVector == 0 || (*it)->Address.PageID == pageID) 		LM old not for independent plane read
					if(planeVector == 0 || (*it)->Address.PageID == pageID || independent_plane_read_enabled)
					{
						//std::cout<<"the pageid is "<<(*it)->Address.PageID<<std::endl;

						//if(independent_plane_read_enabled)
							//std::cout<<"independent plane read is enabled"<<std::endl;
			...
			{
				//std::cout<< "Dispatching " << transaction_dispatch_slots.size() << " transactions to chip " << dieID << std::endl;
